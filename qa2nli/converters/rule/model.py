...
with open(Path(__file__).absolute().parent / 'preps.txt', 'r') as f:
    common_preps = f.read().splitlines()


def is_aux(tok):
    return (tok['form'].lower() in AUX or tok['xpostag'] == 'MD'
            or tok['form'] == 'there')


def is_verb(tok):
    return (tok['xpostag'].startswith('V')
            or tok['upostag'] in ['VERB', 'AUX'] or tok['xpostag'] == 'MD')
...
class Question:
    def __init__(self, question):
        ...
        # Get the subject
        subj_nodes = self._get_children(
            self.root, ['nsubj', 'nsubj:pass', 'csubj'], 'anywhere')
        ...
        # Get auxiliary
        aux_nodes = self._get_children(self.root, ['aux', 'aux:pass'],
                                       'anywhere')
    ...
    def _get_wh_pos(self):
        ...
        for tok in question[wh_idx + 1:]:
            if ((not is_verb(tok)) or tok['id'] in self.possible_gerunds) and (
                ...
                if len(question) > tok['id'] + 1 and question[
                        tok['id'] + 1]['form'] == 'to':
    ...
    def _dangling_prep(self):
        prep = self._get_children(self.root, ['compound:prt', 'obl', 'case'],
                                  'right')
        question = self.question

        for p in prep:
            if p['deprel'] == 'case' and any(
                [tok['head'] == p['id']
                 for tok in question[:self.root['id']]]):

                return p
            elif p['deprel'] == 'case':
                continue
            elif p['xpostag'] in PREPS and len(
                    question) > p['id'] + 1 and question[p['id'] +
                                                         1]['head'] > p['id']:
    ...
    def _wh_is_quantity(self):
        ...
        return (q_length > wh_idx + 1 and wh_lower == 'how'
                and question[wh_idx + 1]['form'] in ['many', 'much'])
    ...
    def _wh_is_happened(self):
        ...
        if len(question) > wh_idx + 1 and question[wh_idx +
                                                   1]['form'] == 'happened':
    ...
    def _aux_precedes_verb(self):
        ...
        if cop is not None and is_verb(
                root) and cop['id'] == root['id'] - 1 - adverbs:

            return True
        elif aux is not None and aux['id'] == root_idx - adverbs - len(
                self.aux_toks):
    ...
    def _get_new_aux_pos(self):
        ...
        for tok in reversed(question[:last_prev_verb]):
            if tok['xpostag'].startswith('RB') or (
                    tok['deprel'] == 'advmod'
                    and tok['head'] in [root_idx, last_prev_verb]):
    ...
    def _get_new_cop_pos(self):
        ...
        if old_pos == lastword_idx:
            return old_pos
        root_mod = self._get_children(root, ['case', 'det', 'amod', 'advmod'],
                                      'left')
    ...
    def swap_aux(self):
        ...
        neworder = [i for i in range(old_pos)] + [i + aux_length for i in range(old_pos, new_pos - aux_length)] \
            + [i for i in range(old_pos, old_pos + aux_length)] + \
            [i for i in range(new_pos, len(question))]
    ...
    def insert_answer(self, a):
        ...
        if startidx == a_pos:
            self.question = question[:startidx] + answer + question[endidx +
                                                                    1:]
        elif startidx < a_pos:
            self.question = question[:startidx] + question[
                endidx + 1:a_pos] + answer + question[a_pos:]
        else:
            self.question = answer + question[:startidx] + question[endidx +
                                                                    1:]
    ...
    def insert_answer_default(self, a):
        ...
        if aux is not None and aux['form'] in AUX_DO and not self.is_do_neg:
            self.change_tense(
                past=(aux['form'] == 'did'), pres_3sg=(aux['form'] == 'does'))
        ...
        if question[startidx]['xpostag'] in PREPS and a.answer[0][
                'xpostag'] not in PREPS:
            a = add_affix([a], [question[startidx]['form'].lower()],
                          question[startidx]['xpostag'], 'left')[0]
            ...
            if wh_lower in PREP_DICT and not (wh_lower == 'how'
                                              and startidx != endidx):
...
class AnswerSpan:
    ...
    def _preprocess(self, answer):
        ...
        if len(answer) > 0 and not answer[0]['xpostag'].startswith('NNP'):
            answer[0]['form'] = answer[0]['form'][0].lower(
            ) + answer[0]['form'][1:]
